...
name: communicate
description: >
  Cross-repo + mesh communication from appsec: file tracked GitHub issues
  ...
  the next step lives outside appsec (a brief for a sibling-repo agent, a
  status ping for a Culture channel, or pulling an issue body + comments
  into context). Issue posts auto-sign with `- appsec (Claude)`; mesh
  messages are unsigned (the IRC nick is the speaker). Not for in-appsec
  ...
  Vendored from guildmaster; see docs/skill-sources.md for divergence notes.
---
