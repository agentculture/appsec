...
description: >
  ...
  defaults records to this agent's PERSONAL, PRIVATE scope (`--scope appsec
...
scope:** the default personal scope is **private** (`--scope appsec
...
- `--scope NAME` / `--visibility public|private` — record scope. **The wrapper
  ...
  (here, `appsec`). Private records are served only to a recall in the same
